...
async function clearFile(){
  try{
    await fs.writeFile('running.pid','', 'utf-8');
    return;
...
export async function startTheEngine(elo: string = '1320'){
    ...
    if(PID === undefined) throw new Error('error in getting PID of spawned process');
    fs.writeFile('running.pid', PID.toString(), 'utf-8');
...
export async function shutEngineDown(){
    ...
    try{
      ...
      console.log(err);
      throw new Error('error in reading pid file for terminating engine');
    }
